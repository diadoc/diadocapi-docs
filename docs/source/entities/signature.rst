...
Диадок сразу покажет отправителю все подписи получателя под входящим документом.

Передать согласующую подпись можно с помощью метода :doc:`../http/PostMessagePatch`: передайте файл подписи в поле ``Signature`` структуры :doc:`../proto/DocumentSignature` и укажите флаг ``IsApprovementSignature = true``.
