...
- При генерации :ref:`титула с МЧД <generate_title_xml_poa>` методом :doc:`../http/GenerateTitleXml`. Метод заполняет поля блока ``Подписант`` данными, полученными из МЧД. Обратите внимание, что метод генерации не заполняет поля машиночитаемой доверенности.
...
Указать МЧД можно следующими способами:

	- указать регистрационный номер МЧД и ИНН доверителя,
	- указать признак «использовать МЧД по умолчанию»,
	- передать файл МЧД и подпись под ней,
	- для акта сверки в формате приказа №405 указать признак ``PowerOfAttorneyToPost.UseXmlContent``, чтобы взять данные МЧД из содержимого документа.
