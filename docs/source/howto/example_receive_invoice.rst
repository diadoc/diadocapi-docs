...
Подтверждения оператора на извещение о получении счета-фактуры
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Сначала необходимо найти входящие счета-фактуры, которые требуется обработать. Последовательность действий аналогична порядку 14Н (см. :ref:`search_invoice`).

Получение счета-фактуры и подтверждения оператора
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Затем необходимо получить найденный СФ :doc:`Invoice <../proto/Entity message>` и подтверждение оператора :doc:`InvoiceConfirmation <../proto/Entity message>`. Последовательность действий аналогична порядку 14Н (см. :ref:`receive_confirmation`).
...
На полученный счет-фактуру Покупатель должен отправить в ответ подписанное извещение :doc:`InvoiceReceipt  <../proto/Entity message>` о получении СФ. Последовательность действий аналогична порядку 14Н (см. :ref:`create_invoice_receipt1`).


Отправка извещения о получении счета-фактуры
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Полученное на предыдущем этапе извещение нужно подписать и отправить. Последовательность действий аналогична порядку 14Н (см. :ref:`send_invoice_receipt1`).
...
Последовательность действий аналогична порядку 14Н (см. :ref:`create_correction_request`).
