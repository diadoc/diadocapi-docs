...
Некоторые документы перед отправкой нужно согласовать с другими сотрудниками. Такие документы можно отправить только тогда, когда эти сотрудники поставят под ним свои подписи.

Чтобы у сотрудников была возможность подписать документ перед отправкой, используйте функцию **отложенной отправки**.

Для этого в структуре :doc:`proto/MessageToPost` выставите флаг ``DelaySend``. При вызове метода :doc:`http/PostMessage` документ с этим флагом будет сохранен без отправки.

Особенности документа с отложенной отправкой:
