...
Особенности сообщений с флагом ``DelaySend``:

- Под документом не может быть подписи или запроса на подпись по доверенности.

- Нельзя изменить содержимое документа и реквизиты получателя.

- Сохраненные документы можно найти с помощью метода :doc:`../http/GetDocuments`. Используйте фильтр ``DocumentStatus = WaitingForSenderSignature`` в запросе.

- Сохраненные документы можно согласовать с помощью метода :doc:`../http/PostMessagePatch` или через веб-интерфейс.

- Если обработка на стороне отправителя больше не требуется, документ можно подписать и отправить с помощью метода :doc:`../http/PostMessagePatch` или через веб-интерфейс.
