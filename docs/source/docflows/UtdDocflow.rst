...
-  :download:`XSD-схема 1-го титула формата УПД <../xsd/ON_SCHFDOPPR_1_995_01_05_01_02.xsd>`;

    -  используется при передаче УПД с функцией СЧФ, ДОП, СЧФДОП;

-  :download:`XSD-схема 2-го титула формата УПД <../xsd/ON_SCHFDOPPOK_1_995_02_05_01_02.xsd>`;
...
#.  Покупатель получает титул продаца УПД (СЧФДОП) и подтверждение оператора и отправляет в ответ подписанные извещение *InvoiceReceipt*\ :sub:`5`\ о получении титула продавца УПД (СЧФДОП) и извещение *InvoiceReceipt*\ :sub:`4`\ о получении подтверждения.
...
Все данные подписанта доставаилсь из сертификата и данных его организации - автоматическое заполнение данных подписанта происходило при заполнении *BoxId* и *Certificate/CertificateThumbprint*.
...
Расширенные данные можно заполнить методом :doc:`../http/utd/ExtendedSignerDetails`.
...
-  структура для описания реквизитов подписанта, используемая в методе :doc:`../http/utd/ExtendedSignerDetails` - :doc:`../proto/utd/ExtendedSignerDetailsToPost`
