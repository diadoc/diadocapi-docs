...
При этом, отправитель документа может явно запросить отправку извещений о получении для документа УПД с функцией ДОП. Для этого при отправке документа нужно указать флаг ``NeedReceipt`` в структуре :doc:`../proto/XmlDocumentAttachment`.
