...
- Загрузить документ как Черновик. В веб-интерфейсе подписать черновик и отправить
- Загрузить документ как Исходящий неотправленный. В веб-интерфейсе подписать документ и отправить
...
---------------------------------------------
Титул продавца доставлен в ящик получателя. Об этом можно узнать при помощи методов чтения ленты новостей GetNewEvents и GetDocflowEvents. Как понять, что это документ именно 820 формата? В ответе каждого из методов чтения ленты новостей возвращается версия документа (поле Version). Для документов 820 формата версия будет начинаться с "utd820_" (например, utd820_05_01_01, utd820_05_01_01_hyphen).

Из ленты новостей можно определить идентификатоы документа MessageId, DocumentId и при необходимости запросить дополнительную информацию по документу, используя методы :http:`GetMessage`, :http:`GetDocument`, :http:`GetDocflows_V3`.

Парсинг документа
-------------------

Получатель документа хочет выполнить парсинг полученного титула и загрузить информацию в свою учетную систему. Для этого необходимо использовать метод :http:`ParseTitleXml`. 

Для того, чтобы распарсить документ, нужно знать тип документа, функцию, версию, номер титула. Тип, функцию и версию можно узнать из ответов методов :http:`GetNewEvents`, :http:`GetMessage`, :http:`GetDocument`, :http:`GetDocflowEvents_v3', :http:`GetDocflows_V3` или использовать метод детектирования :http:`DetectDocumentTypes` для полученного титула продавца. 
...
Для генерации можно использовать один из универсальных методов - :http:`GenerateTitleXml` или :http:`GenerateRecipientTitleXml`. В теле запроса должен содержаться упрощенный XML-файл (UserDataXml), соответствующий XSD-схеме контракта, который мы использовали для генерации титула. В теле ответа будет xml титула покупателя.

Для получения xsd-схемы вызовем метод :doc:`GetDocumentTypes` и найдем интересующие нас тип-функцию-версию. Для титула покупателя УПД в 820 формате возьмем те же значения, что и для титула продавца, но номер титула будет другой:
- TypeNamedId - UniversalTransferDocument
- Function - СЧФДОП
- Version - utd820_05_01_01
...
Готовый титул покупателя можно:
- подписать и сразу отправить продавцу;
- согласовать на своей стороне (через отправку на согласование или постановку на маршрут), а затем подписать и отправить.

Все действия кроме подписания выполняются при помощи метода :http:`PostMessagePatch`. Как сформировать подпись определяет разработчик интеграционного решения.
...
В данной статье не рассмотрены примеры для генерации и отправки извещений о получении. С ними работа осуществляется также, как и для документов в 155 формате: Для генерации xml-файла необходимо использовать метод :http:`GenerateReceiptXml`. Для отправки извещения - метод :http:`PostMessagePatch` и структуру Receipt.
 
 Схемы титулов и UserDataXml
 ---------------------------
 Актуальные xsd-схемы титулов и UserDataXml рекомендуеv gjkexfnm при помощи метода :doc:`GetDocumentTypes`.
 
 Ниже приведены xsd-схемы для версии utd820_05_01_01_hyphen.
 
 //а дальше идут ссылки на файлики
 - xsd 1 титула
 - UserDataXsd для  1 титула
 - xsd 2 титула
 - UserDataXsd для 2 титула
