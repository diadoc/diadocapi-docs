...
Чтобы сгенерировать упрощенный титула ``UserDataXml`` с :doc:`машиночитаемой доверенностью (МЧД) <../howto/powerofattorney>`, нужно заполнить данные подписанта информацией из МЧД. Сделать это можно следующим образом:

- если детали подписанта генерируются по сертификату ``SignerReference``, то для правильного формирования подписанта в титуле нужно заполнить структуру ``PowerOfAttorney``: указать регистрационный номер МЧД и ИНН доверителя или использовать МЧД по умолчанию;
...
Обратите внимание, что метод генерации не заполняет поля машиночитаемой доверенности.
