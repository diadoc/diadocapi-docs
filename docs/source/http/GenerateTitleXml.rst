...
Теперь нужно подготовить контент для титула. Формализованные типы документов описываются XSD-схемами. В XSD-схемах множество элементов, которые могут быть заполнены автоматически за пользователя (сведения об участниках документооборота, версия формата, версия программы и т.п.). Часть данных может быть заполнена только пользователем (продавцы и покупатели, заказчики и исполнители, подписанты, информация о товарах, услугах и т.п.). Для того, чтобы упростить процесс генерации, Диадок позволяет заполнить только «пользовательский» xml-файл, он же UserDataXml, на основе которого метод генерации сформирует основной титул.
