...
Метод отправляет контрагенту пришлашение к партнерским отношениям. С помощью этого метода можно добавить организацию в :doc:`список активных контрагентов <../Counteragents>`. 
...
.. http:post:: /V2/AcquireCounteragent
	...
	:queryparam myDepartmentId: идентификатор подразделения, от имени которого производится инициация отношения партнерства. Параметр может отсутствовать.
	...
	:request Body: Тело запроса должно содержать структуру ``AcquireCounteragentRequest``:
		...
		- Если организация контрагента зарегистрирована в Диадоке, укажите параметр ``OrgId`` контрагента. Он поможет идентифицировать контрагента с помощью КПП. Получить ``OrgId`` огранизации можно с помощью метода :doc:`GetOrganizationsByInnKpp`.
	...
	:statuscode 401: в запросе отсутствует HTTP-заголовок ``Authorization`` или в этом заголовке содержатся некорректные авторизационные данные. // текст может меняться в зависимости от метода
...
Пользователь может работать со список контрагентов организации только если у него есть доступ хотя бы к одному ящику этой организации.
...
.. http:post:: /AcquireCounteragent
	...
	:queryparam comment: текст сообщения для контрагента. Параметр может отсутствовать, длина не должна превышать 2000 символов.
	:queryparam myDepartmentId: идентификатор подразделения, от имени которого производится инициация отношения партнерства. Параметр может отсутствовать.
