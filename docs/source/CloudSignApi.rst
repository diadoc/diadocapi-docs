...
Подписание облачной ЭЦП осуществляется в два этапа.
...
В результате успешного завершения этого этапа вы получите token (идентификатор операции с облачной ЭЦП), а на телефон пользователя будет отправлено SMS-сообщение с кодом подтверждения операции.

Во-вторых, надо вызвать метод :doc:`http/CloudSignConfirm` и передать в него полученный token и SMS-код. Затем, вызывая метод :doc:`http/CloudSignConfirmResult`, дождаться окончания операции и получить либо подписи, либо код ошибки.
